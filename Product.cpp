...
void Product::removeSpecificReview(const std::string& username, const std::string& comment) {
    auto it = std::remove_if(reviews.begin(), reviews.end(), [&](const Review& r) {
        return r.getUsername() == username && r.getComment() == comment;
    });
    if (it != reviews.end()) {
        reviews.erase(it, reviews.end());
        std::cout << "Specific review removed.\n";
    } else {
        std::cout << "No matching review found.\n";
    }
}
