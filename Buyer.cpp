...
void Buyer::removeFromCart(std::shared_ptr<Product> product, int qty) {
    int removed = 0;
    ...
    if (removed > 0) {
        std::cout << "Removed " << removed << "x " << product->getName() << " from cart.\n";
    } else {
        std::cout << "No " << product->getName() << " found in cart to remove.\n";
    }
}
...
std::vector<std::shared_ptr<Product>> Buyer::getPurchaseHistory() {
